...
// TextInPort takes the player's typed words. A live session accepts both typed
// and spoken input, which is the same conversation either way.
func (b *LiveSession) TextInPort() chan<- string { return b.typed }

// SecondaryTextInPort takes the observer's correction. It is a control channel
// rather than a second way to talk, which is why it is the secondary one.
func (b *LiveSession) SecondaryTextInPort() chan<- string    { return b.steer }
