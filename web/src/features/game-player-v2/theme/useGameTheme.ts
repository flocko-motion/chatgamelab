...
export function generateCssVars(theme: GameTheme): Record<string, string> {
  ...
  // Card backgrounds
  const playerBgColor = CARD_BG_COLORS[theme.player.bgColor] || CARD_BG_COLORS.white;
  const gameBgColor = CARD_BG_COLORS[theme.gameMessage.bgColor] || CARD_BG_COLORS.white;
  
  // Font colors
  const playerFontColor = FONT_COLORS[theme.player.fontColor] || FONT_COLORS.dark;
  const gameFontColor = FONT_COLORS[theme.gameMessage.fontColor] || FONT_COLORS.dark;
  
  // Card styling (shared)
  const borderWidth = CARD_BORDER_THICKNESSES[theme.cards.borderThickness] || CARD_BORDER_THICKNESSES.thin;
  ...
  const gameBorderColor = THEME_COLORS[theme.gameMessage.borderColor] || THEME_COLORS.amber;
  
  // Image highlight color (always uses AI border color)
  
  // Status field colors (with safe fallbacks for undefined statusFields)
  const statusBgColor = CARD_BG_COLORS[theme.statusFields?.bgColor] || CARD_BG_COLORS.creme;
  const statusAccentColor = THEME_COLORS[theme.statusFields?.accentColor] || THEME_COLORS.amber;
  const statusBorderColor = THEME_COLORS[theme.statusFields?.borderColor] || THEME_COLORS.amber;
  const statusFontColor = FONT_COLORS[theme.statusFields?.fontColor] || FONT_COLORS.dark;
  
  // Header colors (with safe fallbacks)
  const headerBgColor = CARD_BG_COLORS[theme.header?.bgColor] || CARD_BG_COLORS.white;
  const headerFontColor = FONT_COLORS[theme.header?.fontColor] || FONT_COLORS.dark;
  const headerAccentColor = THEME_COLORS[theme.header?.accentColor] || THEME_COLORS.amber;
  ...
  return {
    ...
    '--game-bg-tint': theme.background.tint === 'warm' ? 'rgba(251, 191, 36, 0.08)'
      ...
      : theme.background.tint === 'dark' ? '#0f0f1a'
      : theme.background.tint === 'black' ? '#000000'
      : theme.background.tint === 'pink' ? 'rgba(236, 72, 153, 0.12)'
      : theme.background.tint === 'green' ? 'rgba(34, 197, 94, 0.10)'
      : theme.background.tint === 'blue' ? 'rgba(59, 130, 246, 0.10)'
      : theme.background.tint === 'violet' ? 'rgba(139, 92, 246, 0.10)'
      : theme.background.tint === 'darkCyan' ? '#0a1a1f'
      : theme.background.tint === 'darkViolet' ? '#1a0f2e'
      : theme.background.tint === 'darkBlue' ? '#0a0f1a'
      : theme.background.tint === 'darkRose' ? '#1a0a0f'
      : 'transparent',
