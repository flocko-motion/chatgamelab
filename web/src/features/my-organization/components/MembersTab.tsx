...
export function MembersTab({ members, isLoading, institution }: MembersTabProps) {
  ...
  const leaveOrganizationMutation = useMutation({
    mutationFn: async () => {
      if (!institutionId || !backendUser?.id) throw new Error('No institution or user');
  ...
  const handleDemote = useCallback((user: ObjUser) => {
    setUserToDemote(user);
    openDemoteModal();
  }, [openDemoteModal]);

  const handleRemove = useCallback((user: ObjUser) => {
  ...
  return (
    <>
      <Stack gap="lg">
        ...
        {(isHead || isStaff) && institutionId && (
          <Card shadow="sm" padding="lg" radius="md" withBorder>
            ...
            <InvitesList institutionId={institutionId} showInstitutionColumn={false} />
          </Card>
        )}
      ...
      <ConfirmationModal
        ...
        confirmColor="violet"
        onConfirm={() => userToPromote?.id && promoteMutation.mutate(userToPromote.id)}
      ...
      <ConfirmationModal
        ...
        confirmColor="gray"
        onConfirm={() => userToDemote?.id && demoteMutation.mutate(userToDemote.id)}
      ...
      <ConfirmationModal
        ...
        message={t('myOrganization.removeConfirm', { name: userToRemove?.name })}
        warning={t('myOrganization.removeWarning')}
        warningColor="orange"
        confirmIcon={<IconTrash size={16} />}
        confirmColor="red"
        onConfirm={() => userToRemove?.id && removeMemberMutation.mutate(userToRemove.id)}
