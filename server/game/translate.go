...
func TranslateGame(ctx context.Context, session *obj.GameSession, game *obj.Game, targetLang string) (*obj.Game, error) {
	...
	if !lang.IsValidLanguageCode(targetLang) {
		return nil, fmt.Errorf("unsupported language code: %s", targetLang)
	}
	...
	// Also translate status field names
	...
	if err != nil {
		return nil, fmt.Errorf("failed to marshal game content: %w", err)
	}
	...
	if err := json.Unmarshal([]byte(translatedJSON), &translatedContent); err != nil {
		return nil, fmt.Errorf("failed to parse translated content: %w", err)
	}
	...
	// Translate status field names
	if len(statusFields) > 0 {
		for i := range statusFields {
			key := fmt.Sprintf("statusField_%d_name", i)
			statusFields[i].Name = getTranslatedField(translatedContent, key, statusFields[i].Name)
		}
