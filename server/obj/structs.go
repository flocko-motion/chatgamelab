...
// Structure: { preset: "space", override: { ...only fields to override... } }
type GameTheme struct {
	Preset   string             `json:"preset"`             // preset name (e.g., "space", "fantasy") or "custom"
	Override *GameThemeOverride `json:"override,omitempty"` // fields that override the preset defaults
}

// GameThemeOverride contains optional overrides for preset themes.
// Only non-empty fields will override the preset defaults.
type GameThemeOverride struct {
	Corners      *GameThemeCorners      `json:"corners,omitempty"`
	Background   *GameThemeBackground   `json:"background,omitempty"`
	Player       *GameThemePlayer       `json:"player,omitempty"`
	GameMessage  *GameThemeGameMessage  `json:"gameMessage,omitempty"`
	Cards        *GameThemeCards        `json:"cards,omitempty"`
	Thinking     *GameThemeThinking     `json:"thinking,omitempty"`
	Typography   *GameThemeTypography   `json:"typography,omitempty"`
	StatusFields *GameThemeStatusFields `json:"statusFields,omitempty"`
	Header       *GameThemeHeader       `json:"header,omitempty"`
	Divider      *GameThemeDivider      `json:"divider,omitempty"`
	StatusEmojis map[string]string      `json:"statusEmojis,omitempty"`
}

type GameThemeCorners struct {
	Style string `json:"style"` // brackets, flourish, arrows, dots, none
...
type GameThemePlayer struct {
	Color          string `json:"color"`     // amber, emerald, cyan, violet, rose, slate, hacker, terminal, brown, pink, orange
	Indicator      string `json:"indicator"` // dot, arrow, chevron, diamond, cursor, underscore, pipe, none
	IndicatorBlink bool   `json:"indicatorBlink"`
	BgColor        string `json:"bgColor"`     // white, creme, dark, black, blue, blueLight, green, greenLight, red, redLight, amber, amberLight, violet, violetLight, rose, roseLight, cyan, cyanLight
	FontColor      string `json:"fontColor"`   // dark, light, hacker, terminal
	BorderColor    string `json:"borderColor"` // amber, emerald, cyan, violet, rose, slate, hacker, terminal, brown, pink, orange
}

type GameThemeGameMessage struct {
	DropCap      bool   `json:"dropCap"`
	DropCapColor string `json:"dropCapColor"` // amber, emerald, cyan, violet, rose, slate, hacker, terminal, brown, pink, orange
	BgColor      string `json:"bgColor"`      // white, creme, dark, black, blue, blueLight, green, greenLight, red, redLight, amber, amberLight, violet, violetLight, rose, roseLight, cyan, cyanLight
	FontColor    string `json:"fontColor"`    // dark, light, hacker, terminal
	BorderColor  string `json:"borderColor"`  // amber, emerald, cyan, violet, rose, slate, hacker, terminal, brown, pink, orange
}

type GameThemeThinking struct {
	Text            string `json:"text"`            // e.g. "The story unfolds..."
	Style           string `json:"style"`           // dots, spinner, pulse, typewriter
	StreamingCursor string `json:"streamingCursor"` // dots, block, pipe, underscore, none
}

type GameThemeTypography struct {
	Messages string `json:"messages"` // serif, sans, mono, fantasy
}

type GameThemeCards struct {
	BorderThickness string `json:"borderThickness"` // none, thin, medium, thick
}

type GameThemeStatusFields struct {
	BgColor     string `json:"bgColor"`     // white, creme, dark, black, blue, blueLight, green, greenLight, red, redLight, amber, amberLight, violet, violetLight, rose, roseLight, cyan, cyanLight
	AccentColor string `json:"accentColor"` // amber, emerald, cyan, violet, rose, slate, hacker, terminal, brown, pink, orange
	BorderColor string `json:"borderColor"` // amber, emerald, cyan, violet, rose, slate, hacker, terminal, brown, pink, orange
	FontColor   string `json:"fontColor"`   // dark, light, hacker, terminal
}

type GameThemeHeader struct {
	BgColor     string `json:"bgColor"`     // white, creme, dark, black, blue, blueLight, green, greenLight, red, redLight, amber, amberLight, violet, violetLight, rose, roseLight, cyan, cyanLight
	FontColor   string `json:"fontColor"`   // dark, light, hacker, terminal
	AccentColor string `json:"accentColor"` // amber, emerald, cyan, violet, rose, slate, hacker, terminal, brown, pink, orange
}

type GameThemeDivider struct {
	Style string `json:"style"` // dot, dots, line, diamond, star, dash, none
	Color string `json:"color"` // amber, emerald, cyan, violet, rose, slate, hacker, terminal, brown, pink, orange
}
